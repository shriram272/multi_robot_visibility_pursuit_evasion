...
# Discretize the path between two points
def discretize(p, p_prime, step_size=0.1):
    max_dist = max(math.dist(p[i], p_prime[i]) for i in range(len(p)))
...
# Extract the solution path from the SG-PEG
...
def visualize_environment(environment, path, starting_positions):
    fig, ax = plt.subplots()
    ...
    for idx, start in enumerate(starting_positions):
        x, y = start
    ...
    if path:
        ...
        for i in range(len(starting_positions)):
            x_coords = [starting_positions[i][0], path[0][i][0]]
            y_coords = [starting_positions[i][1], path[0][i][1]]
...
# Define the starting configuration of the pursuers
starting_configuration = [(2, 8), (8, 2)]

# Create the SG-PEG and run the sampler to generate new vertices
graph = SGPEG(environment, starting_configuration)
sampler = Sampler(environment)

# Add samples to the SG-PEG
for _ in range(50):  # Try to add 50 samples
    sample = sampler.get_sample()
    graph.add_sample(sample)

# Extract and visualize the solution path
solution_path = extract_solution(graph)
visualize_environment(environment, solution_path, starting_configuration)